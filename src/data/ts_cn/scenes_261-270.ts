...
export const scenes_261_270: SceneData = {
  ...
  '262': {
    ...
    options: [
      {text: '（战斗后，假设工匠HP <= 6）', goto: '268'},
      {text: '（战斗后，假设你HP <= 0）', goto: '2'},
      {text: '（战斗后，假设成功逃跑）', goto: '12'},
    ],
  },
  '263': {
    // This scene was already present
  ...
  '264': {
    ...
    checkOption: {
      ...
      success: {text: '进行“理智”检定（成功）', goto: '269'},
      failure: {text: '进行“理智”检定（失败）', goto: '5'},
    },
  },
  '265': {
    id: '265',
  ...
  '266': {
    ...
    options: [
      {text: '进行困难「科学(植物学)」检定', goto: 'S266_BOTANY_CHECK'},
      {text: '扑灭炉火再睡觉', goto: '58'},
      {text: '彻夜不眠', goto: '52'},
      {text: '放松下来，好奇地吸一口烟气', goto: '26'},
    ],
  },
  S266_BOTANY_CHECK: {
    // Helper scene for botany check
    id: 'S266_BOTANY_CHECK',
    story: '你仔细辨认这种粉末...',
    checkOption: {
      check: {
        object: 'skill',
      ...
      success: {text: '科学(植物学)检定成功', goto: '76'},
      // PDF doesn't specify failure path for this check, assuming it means no special outcome, player chooses another option from 266
      // Or, more likely, if check fails, they don't get the info and must choose one of the other 3 options from 266.
      // For now, let's assume failure means they just don't get the info and are back to the options of 266.
      // This is tricky. A simple solution is to make failure go back to 266.
      failure: {text: '科学(植物学)检定失败', goto: '266_POST_BOTANY_FAIL'}, // Go to a scene that re-presents options without the check
    },
  },
  ...
  '268': {
    ...
    options: [
      {text: '向他盘问防腐尸体的事', goto: '20'},
      {text: '丢下他前往其他地方调查', goto: '120'},
    ],
  },
  '269': {
    id: '269',
