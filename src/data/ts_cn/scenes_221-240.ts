...
export const scenes_221_240: SceneData = {
  '221': {
    ...
    options: [
      {text: '问阿博加斯特有关阿布纳基人的事', goto: '227'},
      {text: '问阿博加斯特有关旧日支配者的事', goto: '237'},
      {text: '问阿博加斯特有关烬头村村民的事', goto: '245'},
      {text: '趁还来得及赶快离开', goto: '253'},
    ],
  },
  '222': {
    id: '222',
    ...
    entryEffects: [{type: EffectType.CHANGE_HP, value: '-1'}], // TODO: HP check for goto 13
    options: [{text: '（若HP > 0）继续攀爬', goto: '228'}],
  },
  '223': {
    id: '223',
    story:
  ...
  '225': {
    ...
    checkOption: {
      check: {
        ...
        difficulty: CheckDifficulty.NORMAL, // PDF: "这一次检定你可以将技能值加倍" - This is like a bonus, or easier difficulty.
        // For now, representing as NORMAL, bonus logic would be in reducer or a special check type.
      },
      success: {
        text: '进行“锁匠”检定（成功，打勾）',
        effects: [
          {
            ...
            skillKey: CheckObjectKey.LOCKSMITH,
          },
        ],
        goto: '244',
      },
      failure: {text: '进行“锁匠”检定（失败）', goto: '238'},
    },
  },
  '226': {
    id: '226',
  ...
  '232': {
    ...
    checkOption: {
      ...
      success: {text: '进行“力量”检定（成功）', goto: '244'},
      failure: {text: '进行“力量”检定（失败）', goto: '238'},
    },
  },
  '233': {
  ...
  '235': {
    ...
    options: [
      {text: '（战斗后，假设车夫HP <= 4）', goto: '241'},
      {text: '（战斗后，假设你HP <= 0）', goto: '247'},
    ],
  },
  '236': {
    id: '236',
    ...
    options: [
      {text: '带梅来这里', goto: '242'},
      {text: '返回莱德贝特的房子，但对这件事闭口不提', goto: '157'},
    ],
  },
  '237': {
    id: '237',
    ...
    entryEffects: [
      {type: EffectType.SET_FLAG, flagName: 'FLAG_INCREASE_CTHULHU_MYTHOS_2'},
  ...
  '240': {
    ...
    entryEffects: [
      {type: EffectType.MARK_SKILL_SUCCESS, skillKey: CheckObjectKey.LISTEN},
    ],
    options: [{text: '继续', goto: '234'}],
  },
}
