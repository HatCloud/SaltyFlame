...
export const scenes_121_140: SceneData = {
  '121': {
    ...
    checkOption: {
      check: {
        object: 'skill',
        subObject: CheckObjectKey.TRACK, // Assuming TRACK is the key for 追踪
        difficulty: CheckDifficulty.NORMAL,
      },
      success: {text: '进行「追踪」检定（成功）', goto: '141'},
      failure: {text: '进行「追踪」检定（失败）', goto: '130'},
    },
  },
  '122': {
    id: '122',
  ...
  '125': {
    ...
    checkOption: {
      check: {
        object: 'skill',
        subObject: CheckObjectKey.DISGUISE, // 乔装
        difficulty: CheckDifficulty.HARD,
      },
      success: {text: '进行困难「乔装」检定（成功）', goto: '146'},
      failure: {text: '进行困难「乔装」检定（失败）', goto: '139'},
    },
  },
  '126': {
    id: '126',
  ...
  '127': {
    ...
    options: [
      {text: '跟上这个奇怪的人', goto: '142'},
      {text: '觉得这不明智', goto: '160'},
    ],
  },
  '128': {
    id: '128',
  ...
  '131': {
    ...
    options: [
      {text: '返回莱德贝特的房子确保安全', goto: '157'},
      {text: '直面这些黑影', goto: '163'},
    ],
  },
  '132': {
    id: '132',
  ...
  '133': {
    ...
    checkOption: {
      check: {
        object: 'skill',
        subObject: CheckObjectKey.SPOT_HIDDEN, // 侦查
        difficulty: CheckDifficulty.NORMAL, // PDF says "如果你以前检查过这些书架，你可以获得一颗奖励骰" - this implies a normal check that can be bonused
        // bonusDice: true, // This should be set if the condition is met (previous check)
      },
      success: {text: '进行「侦查」检定（成功）', goto: '147'},
      failure: {text: '进行「侦查」检定（失败）', goto: '140'},
    },
    ...
    entryEffects: [
      {
        ...
        flagName: 'FLAG_INCREASE_NATURAL_WORLD_1', // Reducer needs to handle this flag to increase skill
        // payload: { skillKey: CheckObjectKey.NATURAL_WORLD, amount: 1 } // Optional: pass more data if SET_FLAG supports it
      },
    ],
    options: [{text: '继续', goto: '79'}],
  },
  ...
  '138': {
    ...
    // This requires player to choose a skill to roll, then a check.
    // This is complex. I'll offer options for each skill check.
    options: [
      {text: '尝试「魅惑」说服梅', goto: 'S138_CHARM_CHECK'},
      {text: '尝试「说服」说服梅', goto: 'S138_PERSUADE_CHECK'},
      {text: '尝试「话术」说服梅', goto: 'S138_FASTTALK_CHECK'},
    ],
  },
  '139': {
    id: '139',
  ...
  '140': {
    ...
    options: [{text: '继续', goto: '120'}], // TODO: Player should acquire map item
  },
}
