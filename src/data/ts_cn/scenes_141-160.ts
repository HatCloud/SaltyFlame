...
export const scenes_141_160: SceneData = {
  ...
  '144': {
    ...
    info: '你必须选择使用「汽车驾驶」或「心理学」进行一次检定。如果你选用「汽车驾驶」，你投出的结果需要小于等于技能值。如果你选用「心理学」你需要一次困难成功。这个检定的结果需要小于等于技能值的一半（右上角小方格里的数值）。你只能选择一项技能来检定一次。',
    options: [
      {
        type: 'check',
        text: '尝试「汽车驾驶」检定',
